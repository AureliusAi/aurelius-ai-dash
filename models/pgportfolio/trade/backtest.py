...
import numpy as np
from models.pgportfolio.trade import trader
from models.pgportfolio.marketdata.datamatrices import DataMatrices
from models.pgportfolio.tools.trade import calculate_pv_after_commission
...
class BackTest(trader.Trader):
  ...
  def __init__(self, config, net_dir=None, agent=None, agent_type="nn"):
    trader.Trader.__init__(self, 0, config, 0, net_dir, initial_BTC=1, agent=agent, agent_type=agent_type)
    if agent_type == "nn":
  ...
  def trade_by_strategy(self, omega):
    logger.info(">> the step is: {} / {}".format(self._steps, self._total_steps))
    logger.debug("the raw omega is {}".format(omega))
    ...
    self._last_omega = pv_after_commission * omega * \
                       future_price /\
                       portfolio_change
    logger.info("the portfolio change this period is : {}".format(portfolio_change))
