...
import models.pgportfolio.learn.network as network
...
class NNAgent:
    def __init__(self, config, restore_dir=None, device="cpu"):
        ...
        if restore_dir:
            ...
            self.__net.session.run(tf.compat.v1.global_variables_initializer())
    ...
    def init_train(self, learning_rate, decay_steps, decay_rate, training_method):
        ...
        if training_method == 'GradientDescent':
            train_step = tf.train.GradientDescentOptimizer(learning_rate).minimize(self.__loss, global_step=self.__global_step)
        elif training_method == 'Adam':
            train_step = tf.train.AdamOptimizer(learning_rate).minimize(self.__loss, global_step=self.__global_step)
        elif training_method == 'RMSProp':
            train_step = tf.train.RMSPropOptimizer(learning_rate).minimize(self.__loss, global_step=self.__global_step)
        else:
            raise ValueError()
        return train_step
