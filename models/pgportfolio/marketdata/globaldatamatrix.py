...
class HistoryManager:
    ...
    def get_global_panel(self, start:int, end:int, period:int=300, features:tuple=('close',)) -> pd.DataFrame:
        ...
        logging.info(f"multi index: {multi_index}")
        logging.info(f"type(time_index): {type(time_index)}")
        logging.info(f"type(multi_index): {type(multi_index)}")
        logging.info(f"features: {features}")
        logging.info(f"-----------------------------------------------------------")
        logging.info(f"panel.head(): {panel.head()}")
        logging.info(f"panel.index: {panel.index}")
        logging.info(f"panel.dtypes: {panel.dtypes}")
        logging.info(f"-----------------------------------------------------------")
            ...
            for row_number, coin in enumerate(coins):
                ...
                for feature in features:
                    ...
                    if feature == "close":
                        ...
                        sql = ("SELECT date_norm, SUM(volume) as volume"+
                        ...
                        sql = ("SELECT date_norm, MAX(high) as high" +
                               " FROM (SELECT date+{period}-(date%{period})"
                        ...
                        sql = ("SELECT date_norm, MIN(low) as low" +
                                " FROM (SELECT date+{period}-(date%{period})"
                    ...
                    # print(tabulate(panel_section.head(10), headers='keys', tablefmt='psql'))
                    # print(len(panel_section))
                    panel_section = panel_fillna(panel_section, "both")  # we only want to fill_na on the subsection of the input matrix Xprint(tabulate(panel_section.head(10), headers='keys', tablefmt='psql'))
                    # print(len(panel_section))
                ...
                logging.info(f'>> [{coin}] took: {end_timeit - start_timeit} seconds')
        finally:
            connection.commit()
            connection.close()     
        return panel
        ...
        if not self._online:
            ...
            try:
                ...
                if len(coins_tuples)!=self._coin_number:
                    logging.error("An sqlite error happend")
            finally:
                connection.commit()
                connection.close()
            coins = []
