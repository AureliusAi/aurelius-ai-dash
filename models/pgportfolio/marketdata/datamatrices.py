...
class DataMatrices:
    def __init__(self, start, end, period, batch_size=50, volume_average_days=30, buffer_bias_ratio=0,
        ...
        if data_provider.upper() == "POLONIEX":
            self.__global_data = self.__history_manager.get_global_panel(start,
                                                                         ...
                                                                         period=period,
                                                                         features=type_list)
            logging.info(self.__global_data.shape)
            logging.info(f'num_features = {self.__global_data.shape[1]}')
        ...
        self.__PVM = pd.DataFrame(index=self.__global_data.index.get_level_values(1),
                                  columns=self.__global_data.index.get_level_values(0).unique())
        self.__PVM = self.__PVM.fillna(1.0 / self.__coin_no)
        logging.info(f'Portfolio Vector Memory: PVM(head)')
        logging.info(self.__PVM.head(10))

        self._window_size = window_size
        logging.info(f'_window_size: {self._window_size}')
        self._num_periods = self.__global_data.index.levshape[1]
        logging.info(f'_num_periods: {self._num_periods}')
        self.__divide_data(test_portion, portion_reversed)
        
        self._portion_reversed = portion_reversed
        self.__is_permed = is_permed
        logging.info(f'__is_permed: {self.__is_permed}')

        self.__batch_size = batch_size
        logging.info(f'__batch_size: {self.__batch_size}')
    ...
    def get_submatrix(self, ind):
        #logging.info(f'extracting training-data indicies [{ind} -> {ind+self._window_size+1}]')
